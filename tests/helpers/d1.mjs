 ...
 *
 * Small on purpose: it implements the slice of the D1 client the shop actually
 * uses — prepare/bind/first/all/run and batch — against the same migration SQL
 * that runs in production, so the tests exercise the real queries rather than
 * a mock of them.
 */
...
export function createTestD1({ empty = false } = {}) {
  ...
  // `empty` leaves the database with no tables at all, which is exactly what
  // the one-click deploy hands the Worker.
  if (!empty) {
    // DARKOHL_SCHEMA runs the suite against a schema dumped from a real D1
    ...
    if (process.env.DARKOHL_SCHEMA) {
      sqlite.exec(readFileSync(process.env.DARKOHL_SCHEMA, 'utf8'));
    } else {
